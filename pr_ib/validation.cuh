...
namespace host {
    ...
    void PrValid(Csr<VertexId, SizeT> &csr,  PRTYPE &pr)
    {
        ...
        while(!wl.empty())
        {
            ...
            VertexId destStart = csr.offset[node_item];
            VertexId destEnd = csr.offset[node_item+1];
            float res_owner = h_res[node_item];
            for(int j=0; j<destEnd-destStart; j++)
            {
                VertexId dest_item = csr.indices[j+destStart];
	...
	struct PrRes {
		...
		void print(int permute_idx, PRTYPE &pr)
		{
			...
			if(num_large_epsilon == 0) printf("PASS VALIDATION\n");
    ...
    template <typename VertexId, typename SizeT, typename PRTYPE>
    void PrInitValid(Csr<VertexId, SizeT> &csr,  PRTYPE &pr, int partition_idx, VertexId * new_labels_old)
    {
        float lambda = pr.lambda;
        ...
        MALLOC_CHECK(check_res);
        CUDA_CHECK(cudaMemcpy(check_res, pr.res, sizeof(float)*nodes, cudaMemcpyDeviceToHost));
		for(int i=0; i<pr.totalNodes; i++)
		{
			if(i>=pr.startNode && i<pr.endNode) continue;
			assert(check_res[i] == 0);
		}
        float error=0.0;
        float max_error = 0.0;
        int max_error_idx = -1;
		float total_res_h = 0.0;
		float total_res = 0.0;
        for(int i=pr.startNode; i<pr.endNode; i++)
        {
            if(partition_idx==2 || partition_idx == 3)
            {
                ...
                if(max_error == abs(check_res[i]-h_res[old_vertex]))
                    max_error_idx = i;
				total_res_h += h_res[old_vertex];
				total_res += check_res[i];
            }
            else {
                error = error + abs(check_res[i]-h_res[i]);
                max_error = max(max_error, abs(check_res[i]-h_res[i]));
		...
		if(partition_idx == 2 || partition_idx == 3) {
        	...
        	max_error_idx << " dev res " << check_res[max_error_idx] << " host res " << h_res[new_labels_old[max_error_idx]] << 
			" host total res " <<  total_res_h << " dev total res " << total_res << endl;
        	CUDA_CHECK(cudaMemcpy(check_res, pr.rank, sizeof(float)*pr.nodes, cudaMemcpyDeviceToHost));
        	for(int i=0; i<pr.nodes; i++)
            	if(check_res[i]!=1.0-lambda)
			...
			cout <<"\n["<< pr.my_pe << "] error :" << error << ", max error: "<< max_error << " with index "<< 
        	max_error_idx << " dev res " << check_res[max_error_idx] << " host res " << h_res[max_error_idx] << 
			" host total res " <<  total_res_h << " dev total res " << total_res << endl;
        	CUDA_CHECK(cudaMemcpy(check_res, pr.rank, sizeof(float)*pr.nodes, cudaMemcpyDeviceToHost));
        	for(int i=0; i<pr.nodes; i++)
            	if(check_res[i]!=1.0-lambda)
