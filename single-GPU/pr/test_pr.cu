...
//    CUDA_CHECK(cudaDeviceSetCacheConfig(cudaFuncCachePreferShared));
    cudaOccupancyMaxPotentialBlockSize(&numBlock, &numThread, (void *)MaxCountQueue::_launchWarpPer32Items_minIter_2func<int, int, PageRankFuncWarp<int,int,float>, PushVertices<ROUND_SIZE, int, int, float>, PageRank<int, int, float>>, 0, 900);
    //numThread = 512;
    ...
    << numBlock << "x"<< numThread << " min iter "<< min_iter<< " num queues " << num_queues <<" round size "<< ROUND_SIZE << std::endl;
    ...
    PageRank<int, int, float> pr(csr, lambda, epsilon, min_iter, num_queues);
    ...
    timer.Start();
    pr.PrStart_warp<ROUND_SIZE>(numBlock, numThread);
    timer.Stop();
